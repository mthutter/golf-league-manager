...
export const renderEmailForm = catchAsync(async (req, res, next) => {
  ...
  const members = await all(
    "SELECT CASE WHEN lower(trim(e_mail)) = ? THEN ? ELSE e_mail END AS e_mail FROM members WHERE e_mail IS NOT NULL AND e_mail != '' AND e_mail != 'tbd@tbd.com'",
    ["tonyadkison@yahoo.com", "tonykadkison@yahoo.com"],
  );
