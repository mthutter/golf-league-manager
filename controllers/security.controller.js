...
class SecurityController {
  ...
  #getCleanIp(req) {
    ...
    const isTrustedProxy =
      ...
      ipRangeCheck(directConnection, PRIVATE_RANGES);
  ...
  firewallMiddleware = async (req, res, next) => {
    ...
    const matchesSignature = MALICIOUS_SIGNATURES.some((sig) =>
      lowerPath.includes(sig),
    );
    ...
    if (matchesSignature || matchesPath) {
      ...
      if (
        ...
        logger.error(
          `[SECURITY WARNING] Edge leakage caught on proxy IP: ${cleanIp}.`,
        );
      ...
      if (matchesSignature) {
        try {
          ...
          logger.error(
            `[SECURITY] Immediate signature ban insertion failed for ${cleanIp}:`,
            err,
          );
        }
      ...
      if (matchesPath) {
        ...
        if (strikes >= 3) {
          logger.error(
            `[SECURITY] PERMANENT BLOCK CONFIGURED: ${cleanIp} reached strike maximum.`,
          );
        } else {
          await securityModel.setTransientBan(cleanIp);
          logger.warn(
            `[SECURITY] Transient 24h ban added for ${cleanIp}. Strike ${strikes}/3.`,
          );
        }
        ...
        if (typeof posthogClient !== "undefined" && posthogClient.capture) {
          posthogClient.capture({
            ...
            properties: {
              ...
              action_taken:
                strikes >= 3 ? "PERMANENT_MVC_BAN" : "TRANSIENT_24H_MVC_BAN",
            },
          });
        }
        return this.renderNotFound(res);
  ...
  rateLimiter = rateLimit({
    ...
    handler: (req, res) => {
      res
        .status(429)
        .send("Too many requests from this client. Please try again later.");
    },
  });
}
