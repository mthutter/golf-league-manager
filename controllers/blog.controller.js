...
export const renderPost = catchAsync(async (req, res, next) => {
  ...
  if (!post) {
    logger.warn({ slug }, "Requested blog post slug not found");
    res.status(404);
  ...
  return res.render("blog", { view: "show", post });
});
...
export const renderNewForm = (req, res) => {
  return res.render("blog", { view: "new" });
};
...
export const createPost = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (error.message.includes("UNIQUE constraint failed")) {
      logger.warn({ title }, "Blog creation rejected: Non-unique title string");
