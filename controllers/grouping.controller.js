...
export const showTeeTimes = async (req, res) => {
  try {
    ...
    const totalWeeks = 22;
    const weeks = Array.from({ length: totalWeeks }, (_, i) => ({ id: i + 1 }));
...
export const swapPlayers = async (req, res) => {
  try {
    ...
    if (!weekId || !player1 || !player2) {
      return res.status(400).json({ success: false, error: "Missing required swap parameters." });
    }
    ...
    return res.json({ success: true, message: "Positions swapped successfully!" });
  } catch (err) {
