...
describe("Skins Service Calculation Engine", () => {
  test("should correctly isolate an outright skin and distribute payouts", async () => {
    ...
    assert.strictEqual(results.skinTotals["101"].count, 1, "Player 101 total skins should be 1");
    assert.deepStrictEqual(results.skinTotals["101"].holes, [1], "The skin should be won on Hole 1");
    ...
    assert.strictEqual(results.payoutPerSkin, 10, "Payout per skin should equal $10");

    // Happy Gilmore (ID 102) tied nothing and won nothing
    assert.strictEqual(results.skinTotals["102"], undefined, "Player 102 should not be listed as a winner");
  });
});
