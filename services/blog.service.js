...
`).catch((err) => logger.error("Failed to initialize blog table:", err.message));
...
export async function searchPosts(searchTerm) {
  return all(`SELECT * FROM posts WHERE title LIKE ? OR content LIKE ? ORDER BY created_at DESC`, [`%${searchTerm}%`, `%${searchTerm}%`]);
}
...
export async function createNewPost(title, content, imageUrl, galleryUrls) {
  ...
  return run("INSERT INTO posts (title, slug, content, image_url, gallery_urls) VALUES (?, ?, ?, ?, ?)", [
    title,
    slug,
    content,
    imageUrl || null,
