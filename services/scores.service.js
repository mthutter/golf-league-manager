...
export const getWeeklyBreakdown = async (weekId) => {
    const sql = ` 
        SELECT s.score_id, m.name_first || ' ' || m.name_last AS player_name, s.gross_total, s.net_total, s.stableford_total, s.ctp_points, s.birdie_points, (s.stableford_total + s.ctp_points + s.birdie_points) AS total_points 
        FROM scores s 
